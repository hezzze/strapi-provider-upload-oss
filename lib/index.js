        ...
        return new Promise((resolve, reject) => {
          ...
          ossClient.put(fullPath, fileBuffer)
            .then((result) => {
              log(result);
              if (config.baseUrl) {
                // use http protocal by default, but you can configure it as https protocal
                // deprecate config.domain, use baseUrl to specifie protocal and domain.
                let baseUrl = config.baseUrl.replace(/^\/$/, '');
                if (!/^https?:\/\//.test(baseUrl)) {
                  baseUrl = `http://${baseUrl}`;
                }
                file.url = `${config.baseUrl}/${result.name}`;
              } else {
                file.url = result.url;
              }
